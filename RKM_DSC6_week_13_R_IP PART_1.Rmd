...
dt<- as_tibble(advert_data) # there is suggestion to use as_tibble instead of as.tibble.
dt
```
...
#
duplicate_row <- dt[duplicated(dt),]
duplicate_row
...
#
Numeric<- dt %>% select_if(is.numeric)
Numeric
...
```{r}
#Checking the data types of the columns
Categorical=dt %>% select_if(is.character)
Categorical
```
...
#
boxplot(dt$Daily.Time.Spent.on.Site, main= "Daily.Time.Spent.on.Site boxplot",ylab="Daily.Time.Spent.on.Site", boxwex=0.2)
```
...
#
boxplot(dt$Age, main="Age boxplot",ylab = "Age", boxwex=0.2)
```
...
#
boxplot(dt$Area.Income, main="Area.Income boxplot",ylab = "Area.Income", boxwex=0.2)
```
...
ggplot2::qplot(data = dt, x = Area.Income)
```


```{r}
#checking for outlier
outlier_value <-boxplot.stats(dt$Area.Income)$out
outlier_value
```
...
#
boxplot(dt$Daily.Internet.Usage, main="Daily.Internet.Usage boxplot",ylab = "Daily.Internet.Usage", boxwex=0.2)
```
...
#
#finding the mode
DTS.Mode<-getmode(dt$Daily.Time.Spent.on.Site)
DTS.Mode

```
### 4.1d finding quantile for 'Time.Spent.on.Site' variable
```{r}
DTS_quantile <- quantile(dt$Daily.Time.Spent.on.Site)
DTS_quantile
```
...
Age_Range<-range(dt$Age)
print(Age_Range)
#Finding the variance of the age column
Age_Variance<-var(dt$Age)
print(Age_Variance)
...
Age_Mode<-getmode(dt$Age)
Age_Mode

```
```{r}
#4.2d finding quantile for 'Age' variable
Age_Mode<-quantile(dt$Age)
Age_Mode
```
...
```{r}
City <- dt$City
City_freq<- table(City)
City_freq
barplot(City_freq,xlab ="City", col="grey")
```

###5.1e "Topic.Line" variable frequencty barplot

```{r}
Topic <- dt$Ad.Topic.Line
frequency_Topic <- table(Topic)
frequency_Topic
```
...
DTSS<-dt$Daily.Time.Spent.on.Site
#
...
#Checking the data types of the columns
#
New_numeric<- dt %>% select_if(is.numeric)
New_numeric
...
dt$Ad.Topic.Line <- as.factor(dt$Ad.Topic.Line)
...
#
new_dt <- dt
new_dt$Ad.Topic.Line <- as.numeric(new_dt$Ad.Topic.Line)

new_dt$Country <- as.numeric(new_dt$Country)
...
correlationMatrix <- cor(new_dt[,1:6])
...
```{r}
## Let’s build a correlation matrix to understand the relation between each attributes
corrplot(cor(new_dt), type = 'upper', method = 'number', tl.cex = 0.9)
```
...
shuffle_index <- sample(1:nrow(new_dt))
new_dt <- new_dt[shuffle_index, ]
#
...
new_dt$Daily.Time.Spent.on.Site <- normalize(new_dt$Daily.Time.Spent.on.Site)
new_dt$Age <- normalize(new_dt$Age)
new_dt$Area.Income <- normalize(new_dt$Area.Income)
new_dt$Ad.Topic.Line<- normalize(new_dt$Ad.Topic.Line)
new_dt$Male <- normalize(new_dt$Male)
new_dt$Country <- normalize(new_dt$Country)
#
...
intrain <- createDataPartition(y = new_dt$Clicked.on.Ad, p = 0.7, list = FALSE)
training <- new_dt[intrain,]
testing <- new_dt[-intrain,]
```
...
prop.table(table(new_dt$Clicked.on.Ad)) * 100
...
```{r}
## making predictions
p <- predict(fit, new_dt, type = "class")
#
# comparing predicted values to actual results
pred <- table(p, new_dt$Clicked.on.Ad)
pred
```
